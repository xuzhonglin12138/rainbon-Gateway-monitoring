...
- Rainbond 平台已有 RBDPlugin 启停机制：`POST /v2/platform/plugins/{plugin_name}/status` 会更新 `plugin.rainbond.io/enable` 标签。
...
- 禁用插件后能够关闭采集，避免 APISIX 继续向插件推送日志。
...
Rainbond platform plugin install
  ...
  -> create RBDPlugin rainbond-gateway-monitoring
     label plugin.rainbond.io/enable=true

rainbond-gateway-monitoring backend
  -> read/watch RBDPlugin enable label
...
2. 插件后端启动后读取自己的 `RBDPlugin`，判断 `plugin.rainbond.io/enable`。
3. 如果插件处于启用状态，后端扫描 `ApisixRoute`，找出 Rainbond 管理的 route 所在 namespace 和 `ingressClassName`。
...
1. Rainbond 调用现有插件状态接口，把 `plugin.rainbond.io/enable` 设置为 `false`。
2. 插件后端检测到禁用状态。
3. 后端删除由本插件管理的 `ApisixGlobalRule`。
...
Rainbond 已有插件状态接口继续作为生命周期来源：

| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/v2/platform/plugins/{plugin_name}/status` | 设置 RBDPlugin `plugin.rainbond.io/enable` |

### 4.2 请求/响应结构

插件状态接口由 Rainbond 平台已有实现提供，本插件只读取 RBDPlugin 标签，不要求前端额外调用插件生命周期 API。
...
- `RBDPlugin`：通过固定插件名 `rainbond-gateway-monitoring` 读取自身启停标签。
...
4. 插件启用时，为每组创建或更新 `ApisixGlobalRule`。
5. 插件禁用时，删除带有插件管理 label 的 `ApisixGlobalRule`。
...
#### 5.1.6 与 Rainbond 插件启停结合

Rainbond 当前启停插件时只更新 RBDPlugin label，不直接调用插件后端。因此插件后端应自己读取该 label：

- enabled: `plugin.rainbond.io/enable != "false"`
- disabled: `plugin.rainbond.io/enable == "false"`

该方式不需要修改 rainbond 或 rainbond-console 即可工作。后续如果需要严格卸载清理，可以在 Rainbond core 中为 RBDPlugin 增加 finalizer 或卸载钩子，但不是本阶段必需项。
...
- `deploy/rbdplugin.yaml`：RBAC 从默认更新 `apisixroutes` 调整为管理 `apisixglobalrules`，读取 `apisixroutes`。
...
- 实现内容：记录默认 GlobalRule 方案、启停流程、自动发现、RBAC 和实施计划。
- 验收标准：文档覆盖 Rainbond 插件启停、APISIX GlobalRule、Route 级兼容模式。
...
- 实现内容：增加 `apisixglobalrules` 管理权限和 `rbdplugins` 读取权限，将 `apisixroutes` 改为只读。
...
- 实现内容：读取 RBDPlugin enable label，扫描 route 分组，启用时 upsert，禁用时 delete。
- 验收标准：单元测试覆盖启用、禁用、无 CRD、RBAC 失败、namespace override。
...
| RBDPlugin 类型 | `/Users/guox/Desktop/Project/rainbond/pkg/apis/rainbond/v1alpha1/rbdplugin_types.go` | RBDPlugin spec 和 label 存储位置 |
| 插件启停接口 | `/Users/guox/Desktop/Project/rainbond/api/handler/cluster.go` | `ChangePluginStatus` 更新 RBDPlugin enable label |
| 插件状态代理 | `/Users/guox/Desktop/Project/rainbond-console/console/views/rbd_plugin.py` | console 将启停请求代理到 region API |
